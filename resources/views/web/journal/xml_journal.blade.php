...
@section('content')
    ...
    <div id="header_block_param" style="overflow-x: auto; overflow-y: hidden; max-height: 3.5em">
        <p id="header_doc" style="display: inline-block; max-width: 50%">Журнал Отправки XML</p>
    ...
    <script>
        ...
        function get_table_data(){
            ...
            var width = ($('#main_div').width()-270)/2 + 'px'
            var date_str = $("#period").val().replace(/ /g,'')
            date_str = date_str.split('-')
            $.ajax({
                url: '/xml_journal_data/'+date_str[0]+'/'+date_str[1],
                method: 'GET',
                dataType: 'html',
                async: true,
                success: function(res) {
                    jsTable = jspreadsheet(document.getElementById('main_div'), {
                        ...
                        rowResize: false,
                        columns: [
                            {width:width,type:'text',name:'event',title:'Событие',readOnly:true,},
                            {width:width,type:'text',name:'status',title:'Статус',readOnly:true,},
                            {width:'200px',type:'calendar',name:'timestamp',title:'Время отправки', options: { format:'DD.MM.YYYY HH:mm' },readOnly:true,},
                        ],
